import { Bell, CheckCheck, Trash2, Info, AlertCircle, CheckCircle, GraduationCap } from 'lucide-react';
import { useAuth } from '../../lib/auth';
import { useNotifications } from '../../lib/portal-hooks';
import { supabase } from '../../lib/supabase';
import { PortalCard, PortalPageHeader, PortalButton, PortalLoading, EmptyState, Badge } from '../../components/portal-ui';
...
const typeConfig: Record<string, { icon: typeof Info; color: string; bg: string; variant: 'info' | 'error' | 'success' | 'gold' }> = {
  ...
  academic: { icon: GraduationCap, color: 'text-gold-600', bg: 'bg-gold-100', variant: 'gold' },
};
  ...
  return (
    <div className="animate-fade-in">
      <PortalPageHeader
        ...
        action={
          unreadCount > 0 ? (
            <PortalButton variant="secondary" onClick={markAllRead}>
        ...
        <div className="space-y-3">
          {notifications.map((n) => {
            ...
            return (
              <PortalCard
                key={n.id}
                className={`p-5 ${!n.is_read ? 'border-l-4 border-l-gold-500' : ''}`}
              >
