...
import { useTeacherTimetable } from '../../lib/teacher-hooks';
...
export default function TeacherTimetablePage() {
  const { user } = useAuth();
  const { data: slots, loading } = useTeacherTimetable(user?.id);
  ...
  return (
    <div className="animate-fade-in">
      ...
      {slots.length === 0 ? (
        <EmptyState icon={CalendarClock} title="No classes scheduled" message="Your teaching timetable will appear here once classes are assigned." />
      ) : (
        <>
              ...
              <div className="flex gap-3 overflow-x-auto scrollbar-hide pb-2">
                {slotsByDay[todayIndex]?.slots.map((s) => (
                  <div key={s.id} className="flex-shrink-0 w-56 p-4 rounded-xl bg-ink-900 text-white">
                    <div className="text-xs text-teal-400 font-medium">{s.start_time} — {s.end_time}</div>
                    ...
                    <div className="text-xs text-ink-300 mt-1 flex items-center gap-1.5">
                      <Users size={12} /> {s.class_name || 'All'}
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {slotsByDay.map(({ day, slots }) => (
              <PortalCard key={day} className={`p-5 ${day === days[todayIndex] ? 'ring-2 ring-teal-400' : ''}`}>
                <h3 className="text-sm font-bold text-ink-900 mb-3 flex items-center justify-between">
                  {day}
                  {day === days[todayIndex] && <span className="text-xs text-teal-600 font-medium">Today</span>}
                </h3>
                {slots.length === 0 ? (
                    ...
                    {slots.map((s) => (
                      <div key={s.id} className="p-3 rounded-lg bg-ink-50 hover:bg-ink-100 transition-colors">
                        ...
                        <div className="text-xs text-ink-500 mt-1 flex items-center gap-2">
                          ...
                          <span className="flex items-center gap-1"><Users size={10} /> {s.class_name || 'All'}</span>
                        </div>
                      </div>
                    ))}
                  </div>
