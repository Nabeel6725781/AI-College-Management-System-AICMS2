...
import {
  LayoutDashboard, BookOpen, Users, CalendarCheck, ClipboardList,
  Award, CalendarClock, MessageSquare, BarChart3, User, Settings,
...
const navItems = [
  ...
  { label: 'My Classes', route: '/teacher/classes', icon: BookOpen },
  { label: 'Students', route: '/teacher/students', icon: Users },
  { label: 'Attendance', route: '/teacher/attendance', icon: CalendarCheck },
  { label: 'Marks Entry', route: '/teacher/marks', icon: Award },
  { label: 'Assignments', route: '/teacher/assignments', icon: ClipboardList },
  { label: 'Timetable', route: '/teacher/timetable', icon: CalendarClock },
  { label: 'Messages', route: '/teacher/messages', icon: MessageSquare },
  { label: 'Reports', route: '/teacher/reports', icon: BarChart3 },
  { label: 'Profile', route: '/teacher/profile', icon: User },
  { label: 'Notifications', route: '/teacher/notifications', icon: Bell },
  { label: 'Settings', route: '/teacher/settings', icon: Settings },
];

export default function TeacherLayout({ children, activeRoute }: { children: ReactNode; activeRoute: string }) {
    ...
    <div className="min-h-screen bg-ink-50 flex">
      ...
      >
        <div className="flex items-center justify-between px-6 h-20 border-b border-ink-800">
          <button onClick={() => navigateTo('/teacher/dashboard')} className="flex items-center gap-3">
            ...
            <div className="text-left">
              ...
              <div className="text-[10px] uppercase tracking-[0.15em] text-teal-400">Teacher</div>
            </div>
          </button>
        ...
        <header className="sticky top-0 z-20 bg-white/95 backdrop-blur-md border-b border-ink-100 h-16 flex items-center justify-between px-4 lg:px-8">
          <div className="flex items-center gap-3">
            ...
            <div className="hidden sm:block">
              <span className="text-sm text-ink-500">Welcome,</span>{' '}
              <span className="text-sm font-medium text-ink-900">{user?.email}</span>
            </div>
          </div>
          <button
            onClick={() => navigateTo('/teacher/notifications')}
